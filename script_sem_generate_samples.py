...
def load_info(base_dir, data_dir, patch_dir='patches', fname='info.txt'):
    ...
    return (gIDs, datamap, pid_to_fid), count

def generate_triplet_samples(output_dir, data_info:dict, n_samples, pid_offsets):
...
def generate_matched_pairs(output_dir, data_info:dict, n_samples, pid_offsets):
    data_info_key = list(data_info.keys())
...
def generate_image_retrieval_samples(output_dir, data_info:dict, pid_offsets, n_query_per_group=1):
    #data_info_key = list(data_info.keys())
